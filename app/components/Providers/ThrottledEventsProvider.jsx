...
export default function throttledEventsProvider(Component) {
  return class ThrottledEventsProvider extends PureComponent {
    static displayName = `ThrottledEventsProvider(${Component.displayName || Component.name})`
    ...
    subscribeAll() {
      ...
      Object.keys(handlers).forEach(k => handlers[k].forEach(h => this.subscribeHandler(k, h)))
    }
    ...
    unsubscribeAll() {
      ...
      Object.keys(handlers).forEach(k => handlers[k].forEach(h => this.unsubscribeHandler(k, h)))
    }
    ...
    subscribe = (event, handler, {useRAF = true, targetFPS = 60} = {}) => {
      ...
      const handlerThrottled = this.getThrottledFunction(handler, useRAF, targetFPS)
    ...
    unsubscribe = (event, handler) => {
      ...
      const canUnsubscribe = handlers[event] && handlers[event].indexOf(handler) > -1
